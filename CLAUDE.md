...
8. **امضای APK با keystoreِ ثابتِ `mobile/debug.keystore`.** این فایل کامیت شده و در
   هر بیلد روی `~/.android/debug.keystore` کپی می‌شود تا امضای همهٔ نسخه‌ها یکی بماند
   و آپدیت روی نسخهٔ قبلی بدونِ حذف نصب شود. **هرگز این فایل را حذف یا بازتولید نکن** —
   عوض‌شدنش یعنی کاربر باید دوباره اپ را حذف و نصب کند. (workflowِ `gen-keystore` فقط
   اگر فایل نباشد آن را می‌سازد.)

9. **هر اپ applicationId و برچسبِ یکتا دارد** — در workflow اجباری می‌شود:
   پشتیبان = `ir.dpst.soorin_support` با برچسبِ «پشتیبان سورین»، مشتری =
   `ir.dpst.soorin_portal` با برچسبِ «مشتریان سورین». **این‌ها را یکی نکن**؛ اگر
   applicationIdها یکی شوند، دو اپ روی هم نصب می‌شوند (یکی دیگری را پاک می‌کند).
